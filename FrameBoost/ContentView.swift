        ...
        GeometryReader { geometry in
            NavigationStack {
                ScrollView {
                    ...
                    .padding(.horizontal, max(16, min(28, geometry.size.width * 0.04)))
                    .padding(.top, 18)
                    .padding(.bottom, 36)
                }
    ...
    private var videoCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 14) {
            ...
            PhotosPicker(selection: $selectedItem, matching: .videos) {
                Label(hasVideo ? "Choose another video" : "Choose video", systemImage: "plus.circle.fill")
                    .font(.headline)
    ...
    private var settingsCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Toggle("Preserve original audio", isOn: $model.settings.preserveAudio)
                .disabled(model.isProcessing || model.selectedVideoURL == nil)
            Text("Vertical 9:16 videos stay vertical. Other aspect ratios are preserved without stretching.")
        ...
        VStack(spacing: 14) {
            ...
            if model.isProcessing {
                ProgressView(value: model.progress) {
                    Text("Processing \(Int(model.progress * 100))%").font(.subheadline.weight(.medium))
                }
    ...
    private var resultCard: some View {
        VStack(spacing: 12) {
            ...
            .sheet(isPresented: $showShare) {
                if let output = model.outputURL { ShareSheet(items: [output]).presentationDetents([.medium, .large]) }
            }
        }
        .padding(18).frame(maxWidth: .infinity)
        .background(.regularMaterial, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
